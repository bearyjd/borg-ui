...
Snapshot for whoever picks this up next (human or agent). Last updated 2026-06-04.

## Latest — all merged, nothing in flight

`master` is at `da19ad7`. Everything below is shipped; the only open GUI work is one tracked follow-up (#34).
...
- **Issue housekeeping:** #25 (console-flash / `CREATE_NO_WINDOW`) **closed completed** (validated in #33); **#34 filed** for the lone remaining Tier C bit (tray right-click menu *contents*). #23 (archive virtualization) is now **implemented** on branch `perf/archive-contents-streaming-virtualization` (streaming backend + virtual-scroll frontend; see Open items). VSS stays backlog.
...
`master` has, in order: archive browsing + a production-readiness pass (#22), borg non-interactive hardening (#24), the local-repo drive-letter fix (#31) + non-admin preflight (#32), and the Windows GUI-validation harness + scheduled-backup stderr fix (#33). The cross-platform backup **engine** is well tested, and the **Windows GUI layer has now been validated on real hardware** (PR #33, on the KVM VM): all Tier A/B/C items — keychain→Credential Manager, scheduled-task firing, window/UI/tray render, `--minimized`, console-flash, close-to-tray — **except** the tray right-click menu *contents* (tracked in #34).

- **Verified (Linux CI + local):** `cargo fmt`, `cargo clippy --workspace --all-targets -- -D warnings`, `cargo test --workspace` (223 tests), `svelte-check` (0/0), production frontend build.

...
- **PR #32 is OPEN** (`feat/windows-nonadmin-preflight-and-validation` → `master`): friendlier non-admin local-repo preflight + the edge-validation harness. CI (Frontend + Rust) was **pending** at last check — `gh pr checks 32`; merge once green (`gh pr merge 32 --squash`). Branch is 3 commits: the feature, the 7 devil's-advocate fixes (`bebf5a4`), and the report update. All validated on the live VM: **non-admin 3/3 PASS, admin Failed:0**. Full write-up: `.claude/PRPs/reports/windows-nonadmin-preflight-and-validation-report.md`.
- **Warm smoke VM still up** (`KEEP_VM=1` → container `borgui-smoke-win`). Tear down with `docker compose down` (or `down -v` to drop the persisted volume) from `tests/smoke-windows/` when done — and a `down -v` is exactly what the multi-drive follow-up needs (see open items).
...
- **GUI validation (next unstarted work)** — `.claude/PRPs/plans/windows-gui-validation.plan.md` captures the manual/desktop checks that can't be asserted headlessly: window + tray render, `--minimized` landing in the tray, a scheduled task actually firing the headless run, the console-flash being gone, and the keychain writing to Windows Credential Manager. It is a **plan only — not implemented**. This is the main thing standing between the current state and "validated on real hardware."

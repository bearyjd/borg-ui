...
- [ ] **Desktop notifications** — success/failure toast notifications, configurable in settings
- [ ] **Exclude patterns UI** — backend already supports excludes; add UI on backup + schedule forms with custom + preset patterns (`*.tmp`, `node_modules`, `.git`)
